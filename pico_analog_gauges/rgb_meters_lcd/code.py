...
meter_hl = pwmio.PWMOut(board.GP6, frequency=freq, duty_cycle=duty_min)
meter_hr = pwmio.PWMOut(board.GP7, frequency=freq, duty_cycle=duty_min)
meter_e = pwmio.PWMOut(board.GP17, frequency=freq, duty_cycle=duty_min)
    ...
    try:
        ...
        while available >= 14:
            ...
            if power_hl >= duty_max:
                power_hl = duty_max
            elif power_hl <= duty_min:
                power_hl = duty_min
            meter_hl.duty_cycle = power_hl
            power_hr = power_hr * 120
            if power_hr >= duty_max:
                power_hr = duty_max
            elif power_hr <= duty_min:
                power_hr = duty_min
            meter_hr.duty_cycle = power_hr
            power_e = power_e * 600
            if power_e >= duty_max_e:
                power_e = duty_max_e
            elif power_e <= duty_min:
                power_e = duty_min
            meter_e.duty_cycle = power_e
            available = usb_cdc.data.in_waiting
        g.tick()
        g2.tick()
        g3.tick()
